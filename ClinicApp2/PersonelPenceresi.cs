...
namespace ClinicApp2
{
    public partial class PersonelPenceresi : Form
    {
        ...
        private void button_hastaSilGuncelle_Click(object sender, EventArgs e)
        {
            HastaSilGuncellePenceresi hastasilgun = new HastaSilGuncellePenceresi();
            hastasilgun.Show();
        }

        private void button_randOlustur_Click(object sender, EventArgs e)
        {
        ...
        {
            PersonelPenceresi_Load(sender, e);
        }

        private void PersonelPenceresi_Load(object sender, EventArgs e)
        {
        ...
        private void pictureBox1_Click(object sender, EventArgs e)
        {
            ...
            while (rd.Read())
            {
                if ((textBox1.Text == rd["randevu_id"].ToString() || textBox1.Text == rd["hasta_tc"].ToString() || textBox1.Text == rd["hasta_isim"].ToString() || textBox1.Text == rd["hasta_soyisim"].ToString()))
                {
                    ListViewItem item = new ListViewItem(rd["randevu_id"].ToString());
                    item.SubItems.Add(rd["doktor_isim"].ToString() + " " + rd["doktor_soyisim"].ToString());
                    item.SubItems.Add(rd["hasta_isim"].ToString() + " " + rd["hasta_soyisim"].ToString());
